...
@Service
public class ImageClientB {

    private static final Logger log = LoggerFactory.getLogger(ImageClientB.class);

    private final ImageServiceGrpc.ImageServiceBlockingStub stub;

    public ImageClientB() {
        // Cria o canal manualmente (porta do Service-B)
        ManagedChannel channel = ManagedChannelBuilder
                .forAddress("localhost", 9092) // ajuste para a porta gRPC do Service-B
                .usePlaintext()
                .build();
    ...
    public ImageResponse getImage(String imageId) {
        log.info("Chamando Service-B via gRPC. imageId={}", imageId);
